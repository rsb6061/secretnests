...
</style></head><body>${body}<footer class="footer"><div class="wrap"><strong>SecretNests</strong> · Luxury hotel value, according to people who actually stayed.<br><span class="kicker"><a href="/about">How it works</a> · <a href="/methodology">Methodology</a> · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a> · <a href="/disclosures">Disclosures</a></span></div></footer>
...
async function creatorPage(handle, env){
  ...
  const stays=(await env.DB.prepare(`SELECT s.stay_month,s.trip_context,s.paid_nightly_rate,h.name,h.slug,h.city,h.country,vo.would_pay_again,tr.would_return,tr.review_text FROM stays s JOIN hotels h ON h.id=s.hotel_id LEFT JOIN value_opinions vo ON vo.stay_id=s.id LEFT JOIN trip_reports tr ON tr.stay_id=s.id WHERE s.creator_id=? ORDER BY s.created_at DESC LIMIT 40`).bind(creator.id).all()).results||[];
  const tastes=safeJson(creator.taste_profile_json,[]);
  return page(shell(`<section class="hero" data-autoevent="creator_view" data-creator-id="${attr(creator.id)}" style="padding-bottom:20px"><div class="eyebrow">@${esc(creator.handle)}${creator.is_demo?" · Demo profile":""}</div><h1>${esc(creator.display_name)}</h1><p>${esc(creator.bio||"")}</p>${creator.is_demo?'<div class="notice"><strong>Illustrative demo data.</strong> This profile demonstrates the product and does not claim these are real stays.</div>':""}</section><div class="stats"><div class="stat"><strong>${creator.stay_count||0}</strong><span>stays</span></div><div class="stat"><strong>${creator.report_count||0}</strong><span>trip reports</span></div><div class="stat"><strong>${creator.list_count||0}</strong><span>public lists</span></div><div class="stat"><strong>${creator.booking_count||0}</strong><span>bookings generated</span></div><div class="stat"><strong>${money(creator.earnings||0)}</strong><span>earned</span></div></div><h2>Taste profile</h2><div>${tastes.map(t=>`<span class="pill">${esc(t)}</span>`).join("")||'<span class="muted">Taste profile not added yet.</span>'}</div><section class="section"><h2>Recent stays</h2><div class="grid">${stays.map(s=>`<a class="card stay-card" href="/hotel/${encodeURIComponent(s.slug)}" aria-label="View ${attr(s.name)} hotel page" style="display:block;text-decoration:none;color:inherit;cursor:pointer"><div class="eyebrow">${esc([s.city,s.country].filter(Boolean).join(", "))}</div><h3 style="margin:6px 0 8px">${esc(s.name)}</h3>${s.review_text?`<p style="margin:0 0 12px">${esc(s.review_text)}</p>`:""}<span class="muted">${[s.trip_context?esc(s.trip_context):"",s.stay_month?"Stayed "+esc(s.stay_month):"",s.paid_nightly_rate!=null?"paid "+money(s.paid_nightly_rate):"",s.would_pay_again!=null?"would pay again "+money(s.would_pay_again):"",s.would_return==null?"":s.would_return?"would return":"would not return"].filter(Boolean).join(" · ")}</span></a>`).join("")||'<p class="muted">No public stays yet.</p>'}</div></section><section><h2>Lists</h2><ul class="list">${lists.map(l=>`<li><a href="/@${encodeURIComponent(creator.handle)}/lists/${encodeURIComponent(l.slug)}"><strong>${esc(l.title)}</strong></a><br><span class="muted">${esc(l.description||"")}</span></li>`).join("")||'<li class="muted">No public lists yet.</li>'}</ul></section>`),env,{title:`${creator.display_name} (@${creator.handle}) | SecretNests`,canonical:"/@"+encodeURIComponent(creator.handle),robots:creator.is_demo?"noindex,follow":"index,follow"});
}
...
async function hotelPage(slug, env){
  ...
  const firstParty=(await env.DB.prepare(`SELECT tr.review_text,tr.would_return,tr.published_at,
    ...
    JOIN stays s ON s.id=tr.stay_id
    JOIN creator_profiles cp ON cp.id=tr.creator_id
    LEFT JOIN value_opinions vo ON vo.stay_id=s.id
    WHERE tr.hotel_id=? AND tr.status='published' AND COALESCE(s.verification_method,'')<>'demo'
    ORDER BY tr.published_at DESC,tr.created_at DESC LIMIT 8`).bind(h.id).all()).results||[];
  ...
  const hotelEntityId=ORIGIN+"/hotel/"+h.slug+"#hotel";
  const pageEntityId=ORIGIN+"/hotel/"+h.slug+"#webpage";
  const jsonLd={"@context":"https://schema.org","@graph":[
    {"@type":"WebPage","@id":pageEntityId,"url":ORIGIN+"/hotel/"+h.slug,"name":seo.title,"description":seo.description,"isPartOf":{"@id":ORIGIN+"/#website"},"about":{"@id":hotelEntityId},"mainEntity":{"@id":hotelEntityId},"dateModified":h.updated_at||undefined},
    {"@type":"Hotel","@id":hotelEntityId,"name":h.name,"description":hotelIntro,"url":ORIGIN+"/hotel/"+h.slug,"image":mediaUrl?(mediaUrl.startsWith("http")?mediaUrl:ORIGIN+mediaUrl):undefined,"address":h.formatted_address||h.address||undefined,"telephone":h.phone||undefined,"brand":h.brand_name?{"@type":"Brand","name":h.brand_name}:undefined,"sameAs":h.website?[h.website]:undefined,"review":schemaReviews.length?schemaReviews:undefined},
...
${(latestRate?.booking_url||h.booking_url)?`<p><a class="btn" data-event="outbound_booking_click" data-hotel-id="${attr(h.id)}" href="/out/${encodeURIComponent(h.slug)}" rel="nofollow sponsored">Check booking options</a></p>`:""}</section>${amenities.length?`<section><h2>Amenities</h2><div class="amenity-list">${amenities.slice(0,18).map(x=>`<span class="pill">${esc(x)}</span>`).join("")}</div></section>`:""}${firstPartyBlock}${externalEvidenceBlock}${publicExternalReview}</div>
...
async function methodologyPage(env){
  const jsonLd={"@context":"https://schema.org","@type":"WebPage","name":"SecretNests methodology","url":ORIGIN+"/methodology","about":{"@type":"Thing","name":"Luxury hotel value methodology"}};
  return page(shell(`
<section class="hero"><div class="eyebrow">SecretNests methodology</div><h1>How SecretNests separates a great hotel from a good value.</h1><p><strong>SecretNests does not turn ratings, affiliate prices or outside reviews into a traveler value verdict.</strong> We keep first-party stays, hotel facts, observed or estimated prices and external evidence as separate layers so readers can see what each signal actually means.</p></section>
<section class="section"><div class="grid">
  <div class="card"><h3>What a traveler paid</h3><p>The nightly price a contributor says they paid for a specific stay, with dates, room context, booking channel and meaningful inclusions when provided. A paid rate is historical stay evidence, not a current quote.</p></div>
  <div class="card"><h3>What they would pay again</h3><p>A contributor’s own willingness-to-pay for another comparable stay. It is a first-party value opinion, not a market price, star rating or universal recommendation.</p></div>
  <div class="card"><h3>Would-return intent</h3><p>Whether the traveler says they would return. Return intent is displayed independently from price so an excellent stay can still be described as poor value at a particular rate.</p></div>
</div></section>
<section class="section"><div class="take-card"><div class="eyebrow">Data provenance</div><h2>Four signals, kept separate.</h2><table class="fact-table"><tbody>
<tr><th>First-party traveler data</th><td>Traveler-written stay reviews, paid rates, would-return intent and would-pay-again values submitted to SecretNests.</td></tr>
<tr><th>Observed or estimated prices</th><td>Current or historical price context from booking/provider data or clearly labeled estimates. These prices do not become traveler opinions.</td></tr>
<tr><th>Hotel facts</th><td>Location, brand, amenities and other property facts sourced from trusted hotel/provider information where available.</td></tr>
<tr><th>External traveler evidence</th><td>Outside review or traveler signals shown as external context. They remain attributed to their source and do not change SecretNests first-party fair-value calculations.</td></tr>
</tbody></table></div></section>
<section class="section"><h2>How to read a SecretNests value signal</h2><p>When enough first-party stays exist, SecretNests may summarize the distribution of traveler would-pay-again values and compare that signal with observed or estimated prices. Small samples are labeled as early signals rather than definitive verdicts. Where first-party data is insufficient, the hotel page says so instead of manufacturing a value score.</p><p>Booking links may be affiliate links. Commercial relationships do not change first-party traveler reviews or value opinions. See <a href="/disclosures">disclosures</a> for more.</p></section>
`),env,{title:"SecretNests Methodology: Hotel Reviews, Prices & Value",description:"How SecretNests separates first-party traveler stays, what guests paid, would-pay-again value, hotel facts, price context and external review evidence.",canonical:"/methodology",jsonLd});
}
...
async function sitemap(env){
  const entries=[
    ...
    {url:ORIGIN+"/about"},{url:ORIGIN+"/methodology"},{url:ORIGIN+"/value"},{url:ORIGIN+"/value/under-500"},{url:ORIGIN+"/value/around-1000"},
...
async function route(request,env){
  ...
  if(request.method==="GET" && url.pathname==="/methodology")return methodologyPage(env);
