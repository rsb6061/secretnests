...
const shell = (content) => `<header class="wrap"><a class="brand" href="/">SecretNests</a><nav><a href="/value">Value</a><a href="/destinations">Destinations</a><a href="/brands">Brands</a><a href="/compare">Compare</a><a href="/creators">Travelers</a><a href="/contribute">Contribute</a></nav></header><main class="wrap">${content}</main>`;
...
async function submitTrip(request,env){
  ...
  <div class="hero-actions"><a class="btn" href="${attr(hotelUrl)}">${matchedHotel?"Back to "+esc(matchedHotel.name):"Back to SecretNests"}</a><button class="btn secondary" id="share-take" type="button" data-event="contribution_share" data-hotel-id="${attr(hotelId||"")}">Share your take</button></div></div>
  <script>(function(){var b=document.getElementById("share-take");if(!b)return;b.addEventListener("click",async function(){var text=${JSON.stringify(shareText)},url=${JSON.stringify(ORIGIN)}+${JSON.stringify(matchedHotel?"/hotel/"+matchedHotel.slug:"/")};try{if(navigator.share){await navigator.share({title:"My SecretNests hotel take",text:text,url:url});return}await navigator.clipboard.writeText(text+" "+url);b.textContent="Copied";}catch{}})})();</script>`),env,{title:"Trip received | SecretNests",canonical:"/add-your-trip",robots:"noindex,follow"});
}
